...
1. Configure the router's DHCP reservation (if available) so that the router assigns the same IP address to the specified device using it's physical MAC address
1. Configure the router not to limit the DHCP addresses available, and set the Raspberry PI IP configuration to manual
...
We will install MetalLB as our load balancer controller to enable external access to cluster services. In a cloud scenario, this role is usually taken care of by another service, for example for AKS you can use [AGIC](https://learn.microsoft.com/en-us/azure/application-gateway/ingress-controller-overview) that's using Azure Application Gateway as the ingress controller.
...
Without MetalLB if you create a service of type `LoadBalancer`, the external IP will be stuck in `<pending>` when looking at it via the `kubectl get services -n namespace`.
...
I picked Traefik as my Kubernetes Ingress Controller. This comes built into the K3s bundle but I decided to install it manually to target the latest version available (3.10 at the time of writing) hence I disabled it while installing K3s so, here we are, doing that step manually.

We can use Traefik on Kubernetes in different ways:

- Using standard Kubernetes Ingress (May require several annotations for non-straightforward setup)
- Using Traefik IngressRoute (Custom CRD)
- Using the newer Kubernetes Gateway API (Support is not yet complete at the time of writing, July 2024)

I decided to use the custom CRD approach waiting for the GA support of the Gateway API (a new and improved networking specification that is the successor of the Ingress) because the configuration required is small and resonates well with me.
